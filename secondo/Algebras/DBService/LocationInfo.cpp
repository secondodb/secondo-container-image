        ...
        const std::string& cmpHost) const
{
    ...
    // This comparison is insufficient as there might be DNS names involved.
    // Solution: Resolve both hostnames and compare by IP-address.
    // return host == cmpHost;
    ...
    printf("resolved host to compare: %s\n", resolvedHostToCompare.c_str());

    // Only if both hosts can be resolved and are string-equal, they are considered equal.
    if (resolvedHost != "" && resolvedHostToCompare != "" && (resolvedHost == resolvedHostToCompare))
        isIdentical = true;
...
string LocationInfo::resolveHostToIP(std::string hostname) const
{
    ...
    boost::asio::ip::tcp::resolver::query resolver_query(hostname, "", boost::asio::ip::tcp::resolver::query::numeric_service);

    boost::asio::ip::tcp::resolver resolver(io_service);

    boost::system::error_code error_code;

    boost::asio::ip::tcp::resolver::iterator it = resolver.resolve(resolver_query, error_code);

    // TODO Raise exception
    if (error_code)
    {
        // TODO Raise exception
        // std::cout << "Failed to resolve the DNS entry (" << hostname << ").\n"
