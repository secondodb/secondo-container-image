        ...
        const string& replicaLocationPort)
{
    ...
    print("IWHUGADMK.", replicaLocationPort, std::cout);
    ...
    try
    {
                ...
                = relInfo.nodesBegin(); it != relInfo.nodesEnd(); it++)
        {
            ...
            print("Comparing workers - Operand A: ", location.getHost(), std::cout);
            print("Comparing workers - Operand B: ", replicaLocationHost, std::cout);
            bool isSameWorker = location.isSameWorker(replicaLocationHost, replicaLocationPort);
            print("Are workers equal?: ", isSameWorker, std::cout);
            if (isSameWorker)
            {
                ConnectionID connectionId = it->first;
                print("updateReplicationStatus and setting replicated=true for connection %d", connectionId, std::cout);
